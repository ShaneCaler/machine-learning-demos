...
namespace MLAgents
{
	...
	/// </summary>
	public class SnakeRayPerception : RayPerceptionV2
	{
		Vector2 endPosition;
		RaycastHit2D hit;
		...
		// Test method to gather transform.position information from hit objects
		public List<float> TestPerceive(float rayDistance,
	float[] rayAngles, string[] detectableObjects, bool findX, bool findY)
		{
			testPerceptionBuffer.Clear();
			// For each ray sublist stores categorical information on detected object
			// along with object distance.
			foreach (float angle in rayAngles)
			{
				...
				if (hit)
				{
					for (int i = 0; i < detectableObjects.Length; i++)
					{
						if (hit.collider.gameObject.CompareTag(detectableObjects[i]))
						{
							subList[i] = 1;
							if(findX)
								subList[detectableObjects.Length + 1] = hit.transform.localPosition.x / rayDistance;
							else if(findY)
								subList[detectableObjects.Length + 1] = hit.transform.localPosition.y / rayDistance;
							break;
						}
					}
				}
				...
				testPerceptionBuffer.AddRange(subList);
			}
